...
Mapmaker.RoomTopicView = (function($) {

    var Private = {
        handleInputTopic: function() {
            var topic = this.$topicInput.val();
            this.$topicInput.val('');
            $(document).trigger(Mapmaker.RoomTopicView.events.change + '-' + this.room.room, [topic]);
        }
    };
    ...
    var roomTopicView = function(room) {
        var self = this;
 ...
 * @static
 */
Mapmaker.RoomTopicView.events = {
    change: 'Mapmaker:RoomTopicView:change'
};